...
fetch('/orders/api/sales-data/')
  ...
  data.forEach(d => {
    ...
    d["SL"] = +d["SL"];
    d["Đơn giá"] = +d["Đơn giá"];
    d["Giá Nhập"] = +d["Giá Nhập"];
  });

  // Check date parsing
  const validDates = data.filter(d => d["Thời gian tạo đơn"]).map(d => new Date(d["Thời gian tạo đơn"]));
  ...
  const monthData = d3.rollups(
    ...
    month: `Tháng ${month.toString().padStart(2, '0')}`, 
    total,
    monthNum: month 
  }));
  ...
  const hourlyDailyData = d3.rollups(
    ...
    d => {
      const dateStr = d["Thời gian tạo đơn"];
      if (!dateStr) return "00-1900-01-01";
      const date = new Date(dateStr);
  ...
  const billMonthData = d3.rollups(
    ...
    d => {
      ...
      if (!dateStr) return "00";
      const date = new Date(dateStr);
      return (date.getMonth() + 1).toString().padStart(2, '0'); // 01-12
    }
  ).map(([month, bills]) => ({
    month: month,
  ...
  .catch(function(error) {
    ...
    document.body.innerHTML = '<div style="text-align: center; padding: 50px; font-size: 18px; color: red;">Lỗi khi tải dữ liệu. Vui lòng thử lại sau.</div>';
  });

// Hàm tạo dashboard biểu đồ đường
function createLineChartDashboard(data) {
  // Xử lý dữ liệu theo SQL query - sử dụng năm có nhiều dữ liệu nhất
  const yearCounts = d3.rollups(
    data.filter(d => d['Thời gian tạo đơn']),
    v => v.length,
    d => new Date(d['Thời gian tạo đơn']).getFullYear()
  );
  const mostDataYear = yearCounts.length > 0 ? d3.max(yearCounts, d => d[0]) : new Date().getFullYear();
  
  const billFiltered = data.filter(d => {
    const dateStr = d['Thời gian tạo đơn'];
    if (!dateStr) return false;
    const date = new Date(dateStr);
    return date.getFullYear() === mostDataYear;
  });
...
function createSpendingDistributionChart(data) {
  // Lọc dữ liệu theo năm có nhiều dữ liệu nhất
  const yearCounts = d3.rollups(
    data.filter(d => d['Thời gian tạo đơn']),
    v => v.length,
    d => new Date(d['Thời gian tạo đơn']).getFullYear()
